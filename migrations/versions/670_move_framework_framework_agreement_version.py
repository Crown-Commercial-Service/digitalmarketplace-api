...
def upgrade():
    op.add_column('framework', sa.Column('framework_agreement_details', postgresql.JSON(), nullable=True))
    op.execute("""
    ...
    """)
    op.drop_column('framework', 'framework_agreement_version')


def downgrade():
    op.add_column('framework', sa.Column('framework_agreement_version', sa.VARCHAR(), autoincrement=False, nullable=True))
    op.execute("""
        UPDATE framework SET
            framework_agreement_version = framework_agreement_details->>'frameworkAgreementVersion'
    """)
    op.drop_column('framework', 'framework_agreement_details')
